...
export const RecallGame: React.FC<RecallGameProps> = ({ items: propItems, onComplete }) => {
  ...
  const initGame = () => {
    ...
    const unique = Array.from(
      new Map<number, Subject>(matches.map(item => [item.id!, item])).values(),
    )
  ...
  const handleHint = () => {
    const available = possibleWords.filter(
      w => !foundWords.includes(w.id!) && !revealedHints.some(h => h.id === w.id),
    )
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="mt-6">
            <Button onClick={finishGame} variant="secondary">
              I'm Done
            </Button>
          </div>
