...
export const RadicalCompositionGame: React.FC<RadicalCompositionGameProps> = ({
  ...
  useEffect(() => {
    setHelpSteps([
      {
        title: 'Deconstruct Kanji',
        description: 'Look at the Kanji displayed.',
        icon: Icons.Puzzle,
      },
      {
        title: 'Select Radicals',
        description: 'Choose ALL the radical parts that make up the Kanji.',
        icon: Icons.ListCheck,
      },
      {
        title: 'Learn Composition',
        description: 'Understanding parts helps you learn complex Kanji faster.',
        icon: Icons.Brain,
      },
    ])
  ...
  const initRound = async () => {
    ...
    const candidates = items.filter(
      i => i.subject.component_subject_ids && i.subject.component_subject_ids.length > 1,
    )
    ...
    try {
      ...
      // Combine and shuffle
      const options = [...correctSubjects, ...distractorSubjects].filter(
  ...
  const handleSubmit = () => {
    ...
    const isCorrect =
      question.correctIds.every(id => selectedSet.has(id)) &&
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {question.options.map(opt => {
          ...
          if (submitted) {
            if (isCorrect) styles = 'bg-green-100 border-green-500 text-green-800'
              ...
              styles = 'bg-red-100 border-red-500 text-red-800 opacity-50'
              ...
              <div className="text-3xl font-bold mb-1">
                {opt.characters || (
                  <div className="w-8 h-8">
                    <img
                      src={opt.character_images.find(i => i.content_type === 'image/svg+xml')?.url}
      ...
      <div className="text-center">
        {!submitted ? (
          <Button size="lg" onClick={handleSubmit} disabled={selectedIds.length === 0}>
            Submit
          </Button>
        ) : (
