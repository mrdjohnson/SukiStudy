...
export const CustomSession: React.FC<{ user: User }> = ({ user }) => {
    ...
    const { games, items } = (location.state as { games: string[], items: GameItem[] }) || { games: [], items: [] };
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                {!hasNext && (
                     <div className="text-center mt-8">
                        <Button size="lg" onClick={() => navigate('/session/games')}>Finish Session</Button>
                     </div>
                )}
            </div>
        )
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8">
            <div className="mb-6 flex items-center justify-between">
                <Button variant="ghost" size="sm" onClick={() => navigate('/session/custom')}>Quit</Button>
                <div className="flex gap-1">
                    {gameQueue.map((g, idx) => (
                        <div key={idx} className={`h-2 w-8 rounded-full transition-colors ${idx === completedGames ? 'bg-indigo-600' : idx < completedGames ? 'bg-green-400' : 'bg-gray-200'}`} />
                    ))}
                </div>
                <div className="text-sm font-bold text-gray-500">Round {completedGames + 1} / {gameQueue.length}</div>
            </div>
